...
class RefineIntentSignature(dspy.Signature):
    ...
    the action verb, subject, outcome, and business domain with high confidence.
    The outcome is required — if not explicitly stated, infer a concise and
    reasonable desired result based on the intent and context.
    """
    ...
    intent: ParsedIntent = dspy.OutputField(
        desc=(
            "A fully structured work intent with action, subject, outcome, "
            "and domain classification. Outcome is always required."
        )
    )
